...
export default async (fastify: FastifyInstance) => {
  fastify.get<{ Params: IParams }>('/boards/:boardId/tasks', {
    handler: async (req, reply) => {
      ...
      reply.code(200);
      const tasks = service.getAll(fastify.db, boardId);
      return tasks;
    },
  });
  ...
  fastify.get<{ Params: IParams }>('/boards/:boardId/tasks/:id', {
    handler: async (req, reply) => {
      ...
      reply.code(200);
      const tasks = service.getTask(fastify.db, boardId, id);
      return tasks;
    },
  });
  ...
  fastify.post<{ Params: IParams; Body: TaskType }>('/boards/:boardId/tasks', {
    handler: async (req, reply) => {
      ...
      reply.code(201);
      const tasks = service.add(fastify.db, boardId, body);
      return tasks;
    },
  });
  ...
  fastify.put<{ Params: IParams; Body: TaskType }>('/boards/:boardId/tasks/:id', {
    handler: async (req, reply) => {
      ...
      reply.code(200);
      const renewed = service.update(fastify.db, boardId, id, body);
      return renewed;
    },
  });
  ...
  fastify.delete<{ Params: IParams }>('/boards/:boardId/tasks/:id', {
    handler: async (req, reply) => {
      ...
      await service.remove(fastify.db, boardId, id);
      reply.code(204);
    },
  });
};
